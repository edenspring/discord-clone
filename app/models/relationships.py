...
class Relationship(db.Model):
    ...
    first_user_id = db.Column(db.Integer, db.ForeignKey(
        add_prefix_for_prod('users.id')), nullable=False, primary_key=True)
    second_user_id = db.Column(db.Integer, db.ForeignKey(
        add_prefix_for_prod('users.id')), nullable=False, primary_key=True)
    relationship = db.Column(db.String(20), nullable=False)

    first_user = db.relationship(
        'User', foreign_keys="Relationship.first_user_id", cascade="all,delete")
    second_user = db.relationship(
        'User', foreign_keys="Relationship.second_user_id", cascade="all,delete")
